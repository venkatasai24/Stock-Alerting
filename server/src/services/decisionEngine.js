...
export function evaluateStock(stock, priceData, corpAction = null) {
  ...
  const warning = corpAction?.hasCorporateAction
    ? `⚠️ Recent corporate action: ${corpAction.type} on ${corpAction.date} — price movement may reflect this, not business fundamentals. Verify before acting.`
    : null;

  function result(r) { return warning ? { ...r, warning } : r; }

  // ── ETF: never sell, only hold or accumulate on dips
  if (isETF) {
    if (changeP <= BUY_MORE_DIP) {
      return result({ type: "BUY_MORE", pnlP, longTermView: ltv,
    ...
    return result({ type: "HOLD", pnlP, longTermView: ltv,
  ...
  if (pnlP >= PROFIT_STRONG) {
    if (shares <= 1) {
      return result({ type: "BOOK_PROFIT", pnlP, sharesToSell: 1, longTermView: ltv,
    ...
    return result({ type: "BOOK_PROFIT", pnlP, sharesToSell, longTermView: ltv,
  ...
  if (pnlP >= PROFIT_PARTIAL) {
    ...
    if (shares - sharesToSell <= 1) {
      return result({ type: "HOLD", pnlP, sharesToSell: null, longTermView: ltv,
    ...
    const actualPct = Math.round((sharesToSell / shares) * 100);
    return result({ type: "PARTIAL_SELL", pnlP, sharesToSell, longTermView: ltv,
  ...
  if (pnlP <= STOP_LOSS_ZONE) {
    ...
    if (lowGap === null || lowGap <= 15) {
      ...
      return result({ type: "ACCUMULATE", pnlP, longTermView: ltv, message: note });
    }

    const stopPrice = (price * 0.92).toFixed(2);
    return result({ type: "STOP_LOSS", pnlP, longTermView: ltv,
  ...
  if (weekLow > 0) {
    ...
    if (lowGap <= 5) {
      return result({ type: "NEAR_52W_LOW", pnlP, longTermView: ltv,
  ...
  if (pnlP <= AVG_DOWN_ZONE) {
    ...
    return result({ type: isNearLow ? "ACCUMULATE" : "REVIEW", pnlP, longTermView: ltv,
      message: isNearLow
